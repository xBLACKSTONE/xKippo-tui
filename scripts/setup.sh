...
generate_config() {
  ...
  sed -i.bak -e 's/name = "Cowrie Honeypot"/name = "'"Cowrie Security Monitor"'"/' "$CONFIG_FILE.new"
  sed -i.bak -e 's/auto_detect = true/auto_detect = false/' "$CONFIG_FILE.new"
  ...
  if [ ${#FOUND_LOGS[@]} -gt 0 ]; then
    ...
    sed -i.bak -e '/# log_paths = \[/r '"$CONFIG_FILE.tmp" "$CONFIG_FILE.new"
    rm "$CONFIG_FILE.tmp"
  fi
  
  # Add TTY logs path if found
  if [ -n "$FOUND_TTY_LOGS" ]; then
    sed -i.bak -e 's/# tty_log_path = .*/tty_log_path = "'"$FOUND_TTY_LOGS"'"/' "$CONFIG_FILE.new"
  fi
  
  # Add downloads path if found
  if [ -n "$FOUND_DOWNLOADS" ]; then
    sed -i.bak -e 's/# download_path = .*/download_path = "'"$FOUND_DOWNLOADS"'"/' "$CONFIG_FILE.new"
  fi
  
  # Update UI section
  sed -i.bak -e 's/theme = "default"/theme = "'"$UI_THEME"'"/' "$CONFIG_FILE.new"
  sed -i.bak -e 's/terminal_title = "xKippo - Honeypot Monitor"/terminal_title = "xKippo - Honeypot Security Analyst Platform"/' "$CONFIG_FILE.new"
  ...
  if [ ${#ALERT_COMMANDS[@]} -gt 0 ]; then
    ...
    # Replace placeholder in config file
    sed -i.bak -e '/# on_commands = \[/r '"$CONFIG_FILE.cmd.tmp" "$CONFIG_FILE.new"
  ...
  if [ ${#ALERT_IPS[@]} -gt 0 ]; then
    ...
    # Replace placeholder in config file
    sed -i.bak -e '/# ip_blacklist = \[/r '"$CONFIG_FILE.ip.tmp" "$CONFIG_FILE.new"
    rm "$CONFIG_FILE.ip.tmp"
  fi
  
  # Update GeoIP section
  sed -i.bak -e 's/enabled = true/enabled = '"$GEOIP_ENABLED"'/' "$CONFIG_FILE.new"
  if [ -n "$LICENSE_KEY" ]; then
    sed -i.bak -e 's/# license_key = .*/license_key = "'"$LICENSE_KEY"'"/' "$CONFIG_FILE.new"
    sed -i.bak -e 's/# database_path = .*/database_path = "'"$GEOIP_DIR\/GeoLite2-City.mmdb"'"/' "$CONFIG_FILE.new"
  fi
